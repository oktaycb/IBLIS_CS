...
private:
	...
	std::unordered_map< hash_t, int > m_client_ids;

	// netvar container.
	std::unordered_map< hash_t,		// hash key + tables
		std::unordered_map< hash_t,	// hash key + props
...
public:
	...
	void SetupClassData() {
		...
		// iterate list.
		for (; list != nullptr; list = list->m_pNext)
			m_client_ids[RT_HASH(list->m_pNetworkName)] = list->m_ClassID;
	}

	void StoreDataTable(const std::string& name, RecvTable* table, size_t offset = 0) {
		hash_t	var, base{ RT_HASH(name.c_str()) };
		RecvProp* prop;
		RecvTable* child;
	...
	void StoreDataMap(Address ptr) {
		...
		for (int i{}; i < map->m_size; ++i) {
			...
			// hash var name.
			var = RT_HASH(entry->m_Name);

			// if we dont have this var stored yet.
			if (!m_offsets[base][var].m_offset) {
				m_offsets[base][var].m_datamap_var = true;
				m_offsets[base][var].m_offset = static_cast<size_t>(entry->m_offset);
	...
	// get client id.
	__forceinline int GetClientID(hash_t network_name) {
		return m_client_ids[network_name];
	}

	// get netvar offset.
	__forceinline size_t get(hash_t table, hash_t prop) {
		return m_offsets[table][prop].m_offset;
	}

	// get netvar proxy.
	__forceinline RecvVarProxy_t GetProxy(hash_t table, hash_t prop, RecvVarProxy_t proxy) {
	...
	// set netvar proxy.
	__forceinline void SetProxy(hash_t table, hash_t prop, void* proxy, RecvVarProxy_t& original) {
