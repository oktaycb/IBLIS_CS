...
public:
	...
	__forceinline void Init( const Vector &x, const Vector &y, const Vector &z, const Vector &origin ) {
		m_flMatVal[ 0 ][ 0 ] = x.x;
		m_flMatVal[ 0 ][ 1 ] = y.x;
		m_flMatVal[ 0 ][ 2 ] = z.x;
	...
	__forceinline matrix3x4_t( const Vector &x, const Vector &y, const Vector &z, const Vector &origin ) {
		m_flMatVal[ 0 ][ 0 ] = x.x;
		m_flMatVal[ 0 ][ 1 ] = y.x;
		m_flMatVal[ 0 ][ 2 ] = z.x;
		m_flMatVal[ 0 ][ 3 ] = origin.x;
	...
	__forceinline void SetOrigin( const Vector& p ) {
		m_flMatVal[ 0 ][ 3 ] = p.x;
		m_flMatVal[ 1 ][ 3 ] = p.y;
		m_flMatVal[ 2 ][ 3 ] = p.z;
	}
...
public:
	__forceinline bool get_bone(Vector& out, int bone = 0) {
		if (bone < 0 || bone >= 128)
			return false;
