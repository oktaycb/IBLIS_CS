...
namespace PE {
    ...
    using export_t = std::pair< hash_t, Address >;
    ...
    static __declspec(noinline) Module GetModule(const hash_t name) {
        modules_t modules;
        ...
        for (const auto& m : modules) {
            // compare name hash.
            if (RT_HASH(m.GetName().c_str()) == name)
                return m;
        }
    ...
    static __declspec(noinline) Address GetExport(const Module& module, const hash_t name) {
        ...
        for (size_t i{ 0u }; i < dir->NumberOfNames; ++i) {
            ...
            // compare hash.
            if (RT_HASH(export_name.c_str()) == name) {
                // get export ptr.
    ...
    static __declspec(noinline) exports_t GetAllExports(const Module& module) {
        ...
        for (size_t i{ 0u }; i < dir->NumberOfNames; ++i) {
            ...
            if (export_ptr >= (uintptr_t)dir && export_ptr < ((uintptr_t)dir + data_dir->Size)) {
                ...
                exports.push_back({ RT_HASH(export_name.c_str()), fwd_export_ptr });
            }

            // return ptr to normal export.
            else
                exports.push_back({ RT_HASH(export_name.c_str()), export_ptr });
        }
    ...
    static __declspec(noinline) Address GetExport(const Module& module, const std::string& name) {
        return GetExport(module, RT_HASH(name.c_str()));
    }
}