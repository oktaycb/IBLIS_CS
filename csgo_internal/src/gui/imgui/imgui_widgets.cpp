...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    if (text_end - text > 2000 && !wrap_enabled)
    {
        ...
        // - From this point we will only compute the width of lines that are visible. Optimisation only available when word-wrapping is disabled.
...
bool ImGui::ButtonEx(const char* name, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    static std::unordered_map<ImGuiID, button_animation> button_circle;
    static std::unordered_map<ImGuiID, float> button_hover;
    ...
    // Render
    const ImU32 col = ImColor(45.f / 255.f, 45.f / 255.f, 45.f / 255.f, ImGui::GetStyle().Alpha);
    ...
    ImVec4 hovering_color = ImVec4(0.45f + (0.10f * it_hover->second * 0.4f), 0.45f + (0.10f * it_hover->second * 0.4f), 0.45f + (0.10f * it_hover->second * 0.4f), it_hover->second * 0.4f);
    ImVec4 gradient_color = ImVec4(1.f, 1.f, 1.f, it_hover->second * 0.4f);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(hovering_color), GetStyle().FrameRounding);
...
bool ImGui::TabButtonEx(const char* label, bool* active, const ImVec2& size_arg, bool subtab)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (*active)
        it_hover->second = ImClamp(it_hover->second + (3.f * ImGui::GetIO().DeltaTime * (hovered ? 1.f : -1.f)), 1.0f, 1.f);
    else
    ...
    static std::unordered_map<ImGuiID, ImVec4> hover_color;
    ...
    const ImU32 col = GetColorU32((held && hovered) ? ImVec4(0.08f, 0.08f, 0.14f, 1.00f) : hovered ? ImVec4(0.08f, 0.08f, 0.14f, 1.00f) : ImVec4(0.14f, 0.14f, 0.18f, 1.00f));
    RenderNavHighlight(bb, id);
    PushStyleColor(ImGuiCol_Text, it_hcolor->second);
    RenderTextClipped(ImVec2(bb.Min.x + (subtab ? 0.f : 15.f), bb.Min.y) + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, ImVec2(0.5f, 0.5f), &bb);
    PopStyleColor();
...
bool ImGui::TabButton(const char* label, int number, bool active, const ImVec2& size_arg, bool subtab)
{
    return TabButtonEx(label, &active, size_arg, subtab);
}
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    ImDrawFlags rounding_corners = (other_scrollbar_size <= 0.0f) ? ImDrawFlags_RoundCornersBottomRight : 0;
    ImRect bb;
    if (axis == ImGuiAxis_X)
    {
    ...
    else
    {
        ...
        rounding_corners |= ((window->Flags & ImGuiWindowFlags_NoTitleBar) && !(window->Flags & ImGuiWindowFlags_MenuBar)) ? ImDrawFlags_RoundCornersTopRight : 0;
    }
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    */
    ...
    static std::unordered_map<ImGuiID, float> filled_animation;
    ...
    it_filled->second = ImClamp(it_filled->second + (2.2f * ImGui::GetIO().DeltaTime * ((hovered) ? 1.f : -1.f)), 0.0f, 1.f);

    const ImU32 grab_col = ImColor(0.4f + (it_filled->second * 0.6f), 0.4f + (it_filled->second * 0.6f), 0.4f + (it_filled->second * 0.6f), (0.7f + (it_filled->second * 0.3f)) * ImGui::GetStyle().Alpha);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec4 hover_act = ImColor(70, 70, 70, 255);
    const ImVec4 hover_dis = ImColor(190, 190, 190, 255);
    const ImVec4 active_dis = ImColor(250, 250, 250, 255);
    ...
    const ImVec4 text_act = ImVec4(200 / 255.f, 200 / 255.f, 200 / 255.f, 1.f);
    ...
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second)), true);

    //window->DrawList->AddRect(check_bb.Min - ImVec2(1, 1), check_bb.Max + ImVec2(1, 1), ImColor(25, 25, 25), GetStyle().FrameRounding);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float expected_w = window->Size.x - 15 - 15 + 16;
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : expected_w;
    const ImRect frame_bb(window->DC.CursorPos, ImVec2(window->DC.CursorPos.x + window->Size.x - 15 - 15 + 16, window->DC.CursorPos.y + 30));
    const ImRect total_bb(window->DC.CursorPos, ImVec2(window->DC.CursorPos.x + window->Size.x - 15 - 15 + 16, window->DC.CursorPos.y + 30));
    ...
    static std::unordered_map<ImGuiID, float> open_animation;
    ...
    ImGui::RenderFrame(total_bb.Min, total_bb.Max, ImColor(36.f / 255.f, 36.f / 255.f, 36.f / 255.f, ImGui::GetStyle().Alpha * 150.f / 255.f), true, 0.f);
    window->DrawList->AddRect(total_bb.Min, total_bb.Max, ImColor(0.6f + (it_filled->second * 0.4f), 0.6f + (it_filled->second * 0.4f), 0.6f + (it_filled->second * 0.4f), ImGui::GetStyle().Alpha * 1.f), 0.f);
    window->DrawList->AddLine({ total_bb.Max.x - 20 - ImGui::CalcTextSize(preview_value).x - 10, total_bb.Min.y + 9 }, { total_bb.Max.x - 20 - ImGui::CalcTextSize(preview_value).x - 10, total_bb.Max.y - 9 }, ImColor(193.f / 255.f, 193.f / 255.f, 193.f / 255.f, (75.f / 255.f) * ImGui::GetStyle().Alpha), 1);

    if (label_size.x > 0.0f)
        window->DrawList->AddText(ImVec2(total_bb.Min.x + 10, total_bb.Min.y + 15 - ImGui::CalcTextSize(label).y / 2), ImColor(0.6f + (it_filled->second * 0.4f), 0.6f + (it_filled->second * 0.4f), 0.6f + (it_filled->second * 0.4f), ImGui::GetStyle().Alpha * 1.f), label);
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddText(ImVec2(total_bb.Max.x - 20 - ImGui::CalcTextSize(preview_value).x, total_bb.Min.y + 15 - ImGui::CalcTextSize(preview_value).y / 2), ImColor(0.6f + (it_filled->second * 0.4f), 0.6f + (it_filled->second * 0.4f), 0.6f + (it_filled->second * 0.4f), ImGui::GetStyle().Alpha * 1.f), preview_value);
    ...
    it_open->second = ImClamp(it_open->second + (5.f * ImGui::GetIO().DeltaTime * (popup_open ? 1.f : -1.f)), 0.01f, 1.f);
    ...
    if (it_open->second < 0.99f)
        PushStyleVar(ImGuiStyleVar_ScrollbarSize, -1.f);
    ...
    else
    {
        ...
        ImGui::SetNextWindowSizeConstraints(ImVec2(w, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)) * it_open->second);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = ImGui::CalcTextSize(label, NULL, true);
    const ImRect frame_bb(ImVec2(window->DC.CursorPos.x + 4, window->DC.CursorPos.y), ImVec2(window->DC.CursorPos.x + 286, window->DC.CursorPos.y + 30));
    ...
    //ImGui::RenderFrame(frame_bb.Min, frame_bb.Max + ImVec2(15,0), ImColor(25, 25, 25, 100), true, 0.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled({ frame_bb.Min.x + 20 - 3, frame_bb.Min.y + 25 - 1 - 1 }, ImVec2(frame_bb.Max.x - 15 + 1, frame_bb.Max.y + 1 - 1), ImColor(18.f / 255.f, 18.f / 255.f, 18.f / 255.f, (100/255.f) * ImGui::GetStyle().Alpha), style.GrabRounding);//slider background shadow

        window->DrawList->AddRectFilled({ frame_bb.Min.x + 20, frame_bb.Min.y + 25 + 1 - 1 }, ImVec2(grab_bb.Max.x - 15, grab_bb.Max.y + 1 - 1), ImColor(1.f, 1.f, 1.f, ImGui::GetStyle().Alpha), style.GrabRounding);//slider
    }

    static std::unordered_map<ImGuiID, float> slider_hover;

    // hover animation

    auto it_hover = slider_hover.find(id);
    if (it_hover == slider_hover.end())
    {
        slider_hover.insert({ id, {0.f} });
        it_hover = slider_hover.find(id);
    }

    it_hover->second = ImClamp(it_hover->second + 2.5f * ImGui::GetIO().DeltaTime * (hovered ? 1.f : -1.f), 0.f, 1.f * ImGui::GetStyle().Alpha);
    ImColor hover_color = ImVec4(1.f, 1.f, 1.f, it_hover->second * 1.f * ImGui::GetStyle().Alpha);
    ImColor text_color = ImVec4(0.7f + 0.3f * it_hover->second, 0.7f + 0.3f * it_hover->second, 0.7f + 0.3f * it_hover->second, 1.f * ImGui::GetStyle().Alpha);

    window->DrawList->AddText(ImVec2(frame_bb.Min.x + 5 + 1, frame_bb.Min.y + 24 - ImGui::CalcTextSize("<").y / 2 + 2), hover_color, "<");//< decrease text
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - 5 - 6, frame_bb.Min.y + 24 - ImGui::CalcTextSize(">").y / 2 + 2), hover_color, ">");//> increase text

    std::string labelch = label; if (ImGui::CalcTextSize(label).x > 10) labelch += " | ";

    window->DrawList->AddText(ImVec2(frame_bb.Min.x + 20 - 3 + ImGui::CalcTextSize(labelch.c_str()).x, frame_bb.Min.y + 5), text_color, value_buf);//value text

    window->DrawList->AddText(ImVec2(frame_bb.Min.x + 20 - 3, frame_bb.Min.y + 5), text_color, labelch.c_str());//label

    ImGui::Spacing(); ImGui::Spacing();

    IMGUI_TEST_ENGINE_ITEM_INFO(id, labelch.c_str(), window->DC.ItemFlags);
    return value_changed;
}
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_hover->second = ImClamp(it_hover->second + (3.f * ImGui::GetIO().DeltaTime * ((selected || hovered) ? 1.f : -1.f)), 0.f, 1.f);

    const ImVec4 text_dis = ImColor(200, 200, 200, 255);
    const ImVec4 text_act = ImColor(255, 255, 255, 25);

    const ImVec4 hover_dis = ImColor(31, 31, 31, 255);
    const ImVec4 hover_act = ImColor(45, 45, 45, 255);
    ...
    it_filled->second = ImClamp(it_filled->second + (2.2f * ImGui::GetIO().DeltaTime * (((selected || hovered)) ? 1.f : -1.f)), 0.0f, 1.f);
    ...
    RenderFrame(bb.Min, bb.Max, col, false, 0.0f);//highlight
