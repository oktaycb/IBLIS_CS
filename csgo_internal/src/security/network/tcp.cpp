...
void tcp::client::start(const std::string_view server_ip, const uint16_t port) {
	...
	if (ret != 1) {
		LOG_ERROR(XOR("failed to load to ca.\n"));
		return;
	}
	...
	if (ret != 0) {
		LOG_ERROR(XOR("failed to initialize WSA.\n"));
		return;
	}

	m_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	if (m_socket == -1) {
		LOG_ERROR(XOR("failed to create socket.\n"));
		return;
	}
	...
	ret = connect(m_socket, reinterpret_cast<sockaddr*>(&server_addr),
		sizeof(server_addr));
	if (ret < 0) {
		LOG_ERROR(XOR("failed to connect to server.\n"));
		return;
	}
	...
	if (ret != 1) {
		...
		//LOG_ERROR(XOR("secure connection failed, code {} \n"), ret);
		return;
	}
